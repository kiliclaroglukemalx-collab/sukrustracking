...
import { saveReportData, getReportData } from "@/lib/actions";
...
export default function YatirimPerformansPage() {
  const { role, methods } = useStore();
  const [reportYatirimData, setReportYatirimData] = useState<KasaCardData[]>([]);
  ...
  useEffect(() => {
    setHydrated(true);
    fetchSnapshots();
    getReportData("yatirim").then((r) => {
      if (r?.processedData?.length) setReportYatirimData(r.processedData);
    });
  }, []);
  ...
  const handleGunlukUpload = useCallback(async (file: File) => {
    try {
      ...
      await saveReportData("yatirim", rows, processed);
      setReportYatirimData(processed);
  ...
  const handleGecmisUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      const label = isSingleDay
        ? `${gecmisEnd} tarihli veri kaydedildi`
  ...
  /* ─── Derived data ─── */
  const aktifKasa = useMemo(() => reportYatirimData.filter((k) => k.toplamBorc > 0 || k.toplamKredi > 0), [reportYatirimData]);
  ...
  const handleGunlukCopy = useCallback(async () => {
    const t = generateGunlukMetni(reportYatirimData);
    if (!t) return;
    await copyText(t);
