...
export default function AnalizPage() {
  ...
  const handleKasaCopy = useCallback(async () => {
    const metin = generateKasaMetni(kasaData);
    if (!metin) return;
    try {
  ...
  async function fetchSnapshots() {
    try {
      const res = await fetch("/api/haftalik-kasa");
      const json = await res.json();
      if (json.snapshots) {
        setSnapshots(json.snapshots);
      }
    } catch (err) {
      console.error("[AnalizPage] Haftalik snapshot hatasi:", err);
    }
  }
    ...
    <div className="min-h-screen" style={{ background: "#F6F8FB" }}>
      ...
      {(hasKasaData || hasSnapshots) && (
        <div className="mx-auto max-w-[1400px] px-6 pt-6">
          <div className={`grid gap-6 ${hasKasaData && hasSnapshots ? "lg:grid-cols-[3fr_2fr]" : ""}`}>
            ...
            {hasKasaData && (
              <div className="rounded-2xl border border-[#E6EAF0] bg-white">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-lg border px-3 py-2 text-[11px] font-semibold transition-all ${
                      kasaCopied
                        ...
                        : "border-[#E6EAF0] bg-white text-neutral-500 hover:border-neutral-300 hover:text-neutral-700"
                    }`}
                  >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b border-[#E6EAF0] bg-[#F6F8FB]">
                    ...
                    <tbody>
                      {aktifKasa.map((k, i) => {
                        ...
                        return (
                          <tr key={k.id} className={`border-b border-[#E6EAF0]/60 transition-colors hover:bg-[#F6F8FB]/60 ${i % 2 === 0 ? "" : "bg-[#FAFBFD]"}`}>
                ...
                {yediGunOrt.size > 0 && (
                  <div className="border-t border-[#E6EAF0] px-5 py-2">
                    ...
                    {snapshots.map((sn) => {
                      ...
                      return (
                        <div key={sn.snapshot_date} className="flex items-center gap-2">
                          ...
                          <div className="relative h-5 flex-1 overflow-hidden rounded-md bg-[#F6F8FB]">
                      ...
                      <table className="w-full text-left">
                        ...
                        <tbody>
                          {weeklyMethods.map((m, i) => (
                            <tr key={m.name} className={`border-b border-[#E6EAF0]/40 ${i % 2 !== 0 ? "bg-[#FAFBFD]" : ""}`}>
      ...
      {loading && (
        <div className="flex min-h-[60vh] flex-col items-center justify-center">
          <div className="mb-4 h-8 w-8 animate-spin rounded-full border-2 border-neutral-200 border-t-[#1E5EFF]" />
          <p className="text-sm text-neutral-400">Analiz raporu yukleniyor...</p>
        </div>
      )}
      ...
      {!loading && !report && (
        <div className="flex min-h-[60vh] flex-col items-center justify-center px-6 text-center">
          <BarChart2 className="mb-4 h-12 w-12 text-neutral-200" strokeWidth={1} />
          <h2 className="mb-2 text-lg font-bold text-neutral-700">Henuz Analiz Raporu Uretilmedi</h2>
          ...
          <Link href="/raporlar" className="mt-6 rounded-xl border border-[#E6EAF0] bg-white px-6 py-2.5 text-xs font-semibold text-neutral-500 transition-colors hover:bg-neutral-50">
            Raporlara Don
          </Link>
        </div>
      )}
